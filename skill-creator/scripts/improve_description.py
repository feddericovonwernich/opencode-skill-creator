...
by calling OpenCode as a subprocess.
"""
...
from scripts.opencode_runtime import run_opencode_text
from scripts.utils import parse_skill_md


def _call_opencode(prompt: str, model: str | None, run_dir: Path, timeout: int = 300) -> str:
    """Run `opencode run` and return the text response."""
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    text = _call_opencode(prompt, model, run_dir=run_dir)
    ...
    # SDK path did this as a true multi-turn; `opencode run` is one-shot, so we
    ...
    if len(description) > 1024:
        ...
        shorten_text = _call_opencode(shorten_prompt, model, run_dir=run_dir)
